...
export function generateSettings() {
  ...
  this._.form.settings.menu.playbackSpeed = new SettingsRadioButtons("Скорость воспроизведения");
  this._.form.settings.menu.info = new SettingsPage("Информация о плеере");
  this._.form.settings.menu.info.Content = createElement("div", {blockName: "info"}, (el) => {
      let authorBlock = createElement("div", {style: "display: flex; padding-bottom: 12px"}, (bl) => {
        ...
        const infoBlock = createElement("div", {style: "line-height: 20px; padding: 4px 8px"}, (inf) => {
  ...
  this._.form.settings.title = createElement("div", {style: "mjs__settingsHeader-title"});
  this._.form.settings.header = createElement("div",{class: "mjs__settingsHeader"},(el) => {
      el.addEventListener("click", () => {resetMenu.call(this);});
    }
  );
  ...
  for (let speed of [0.5, 1, 1.5, 2]) {
    this._.form.settings.menu.playbackSpeed.appendButton(speed + "x", ()=>{
      setSpeed.call(this, speed);
    })
  }
  this._.form.settings.menu.playbackSpeed.Buttons[1].click();
  ...
  // AppendMenus
  this._.form.settings.body = createElement("div", {class: "mjs__settingsBody"});
